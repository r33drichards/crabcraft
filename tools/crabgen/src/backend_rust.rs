...
//!                    the `<World>Impl` trait, `setup(&mut Registry)` adapter
//!                    registration, typed mesh wrappers.
//! - src/lib.rs       GENERATED every regen (thin: mod gen; mod app; the
//!                    crab_sdk::export_abi! invocation). Like README.md it
//!                    lives outside gen/ but is crabgen-owned.
...
//! - build.sh         SCAFFOLD-ONCE: rustup-from-nix wasip1 build + SIMD
//!                    tripwire.
...
/// Matches the generated tuple-temp pattern (x0, x1, ...).
...
/// Line writer with 4-space indentation. `line` may carry embedded newlines
...
impl W {
    ...
    fn line(&mut self, s: impl AsRef<str>) {
        let s = s.as_ref();
        if !s.is_empty() {
            self.buf.push_str(&pad(self.ind));
            self.buf.push_str(s);
        }
        self.buf.push('\n');
    }
...
impl<'a> Gen<'a> {
    ...
    fn rust_ty(&self, ty: &Ty, qual: &str) -> Result<String> {
        Ok(match ty {
            Ty::List(t) => format!("Vec<{}>", self.rust_ty(t, qual)?),
            Ty::Option(t) => format!("Option<{}>", self.rust_ty(t, qual)?),
            Ty::Tuple(ts) => match ts.len() {
                0 => "()".to_string(),
                1 => format!("({},)", self.rust_ty(&ts[0], qual)?),
                _ => format!(
                    "({})",
                    ts.iter()
                        .map(|t| self.rust_ty(t, qual))
    ...
    fn type_tree(&self, ty: &Ty, ind: usize) -> Result<String> {
        let p0 = pad(ind);
        let p1 = pad(ind + 1);
        Ok(match ty {
            Ty::List(t) => format!("Type::List(Box::new({}))", self.type_tree(t, ind)?),
            Ty::Option(t) => format!("Type::Option(Box::new({}))", self.type_tree(t, ind)?),
            Ty::Tuple(ts) if ts.is_empty() => "Type::Tuple(Vec::new())".to_string(),
            Ty::Tuple(ts) => {
                let items = ts
                    .iter()
                    .map(|t| Ok(format!("{p1}{},\n", self.type_tree(t, ind + 1)?)))
            ...
            Ty::Result(ok, errt) => {
                let side = |t: &Option<Box<Ty>>| -> Result<String> {
                    Ok(match t {
                        Some(t) => format!("Some(Box::new({}))", self.type_tree(t, ind + 1)?),
                        None => "None".to_string(),
                    })
                };
                format!(
    ...
    fn lower_fallible(&self, ty: &Ty) -> Result<bool> {
        Ok(match ty {
            ...
            Ty::Tuple(ts) => ts
                ...
                .any(|b| b),
            Ty::Result(ok, errt) => {
                ok.as_deref().map_or(Ok(false), |t| self.lower_fallible(t))?
                    || errt
                        .as_deref()
        ...
        Ok(match ty {
            ...
            Ty::Tuple(ts) => {
                ...
                let items = ts
                    .iter()
                    .zip(&names)
                    .map(|(t, n)| Ok(format!("{p2}{},\n", self.lower(t, n, ind + 2)?, p2 = pad(ind + 2))))
                    .collect::<Result<String>>()?;
                format!(
                    "{{\n\
                     {p1}let {pattern} = {expr};\n\
    ...
    fn emit_named_ty(&self, w: &mut W, iface: &Iface, t: &NamedTy) -> Result<()> {
        ...
        match &t.ty {
            ...
            other => {
                ...
                w.line(format!("pub type {p} = {};", self.rust_ty(other, "")?));
                w.line("");
            }
        }
        ...
        w.line(self.type_tree(&t.ty, w.ind)?);
        w.close("}");
        w.line("");
        Ok(())
    }
    ...
    fn emit_record(
        ...
        for (n, ft) in fields {
            w.line(format!("pub {}: {},", rust_ident(n), self.rust_ty(ft, "")?));
        }
        w.close("}");
        w.line("");
    ...
    fn emit_variant(
        ...
        for (c, payload) in cases {
            match payload {
                ...
                Some(pt) => w.line(format!("{}({}),", rust_pascal(c), self.rust_ty(pt, "")?)),
            }
        }
        w.close("}");
        w.line("");
    ...
    fn method_sig(&self, f: &Func, qual: &str) -> Result<String> {
        ...
        for (n, t) in &f.params {
            params.push(format!("{}: {}", rust_param(n), self.rust_ty(t, qual)?));
        }
        let ret = match self.classify(f)? {
            Ret::None => "Result<(), String>".to_string(),
            Ret::Plain(t) => format!("Result<{}, String>", self.rust_ty(t, qual)?),
            Ret::ResStr { ok, .. } => match ok {
                Some(t) => format!("Result<{}, String>", self.rust_ty(t, qual)?),
                None => "Result<(), String>".to_string(),
            },
            Ret::ResTyped(t) => format!("Result<{}, String>", self.rust_ty(t, qual)?),
        };
        Ok(format!(
            "fn {}({}) -> {ret}",
    ...
    fn mod_rs(&self) -> Result<String> {
        ...
        for f in &export.funcs {
            ...
            if f.params.is_empty() {
                ...
                w.open("vec![");
                for (_, t) in &f.params {
                    w.line(format!("{},", self.type_tree(t, w.ind)?));
                }
                w.close("],");
            }
        ...
        out.push_str("// generated types/consts/wrappers.\n");
    ...
    fn emit_mesh_wrapper(&self, w: &mut W, iface: &Iface, is: &str, f: &Func) -> Result<()> {
        ...
        for (n, t) in &f.params {
            sig_params.push(format!("{}: {}", rust_param(n), self.rust_ty(t, "")?));
        }
        let sig_ret = match &ret {
            Ret::None => "Result<(), String>".to_string(),
            Ret::Plain(t) | Ret::ResTyped(t) => {
                format!("Result<{}, String>", self.rust_ty(t, "")?)
            }
            Ret::ResStr { ok, .. } => match ok {
                Some(t) => format!("Result<{}, String>", self.rust_ty(t, "")?),
                None => "Result<(), String>".to_string(),
            },
        };
...
fn add_workspace_member(root_cargo: &Path, member: &str) -> Result<()> {
    ...
    let start = src
        .find("members")
        .ok_or_else(|| anyhow!("{}: no workspace members array", root_cargo.display()))?;
    let open = src[start..]
        .find('[')
        .map(|i| start + i)
