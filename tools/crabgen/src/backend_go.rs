...
fn go_param(kebab: &str) -> String {
    ...
    if PARAM_AVOID.contains(&s.as_str()) || is_temp_shaped(&s, GO_TEMP_PREFIXES) {
        s.push('_');
    }
    s
}

/// The temp-name prefixes this emitter generates (x0, n1, i2, ok0, isErr0…).
const GO_TEMP_PREFIXES: &[&str] = &["x", "n", "i", "ok", "isErr"];

// ---------------------------------------------------------------------------
// emission plumbing (the shared W from emit.rs, tab-indented — gofmt's)
// ---------------------------------------------------------------------------
