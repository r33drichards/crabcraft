...
// WIT world has imports (same split as the Go template's mesh.go /
// mesh_wasm.go). asc additionally tree-shakes entry-unreachable code, so
// the @external declaration below lands in the wasm import section — and
// the host is required to provide it — only once the impl actually calls a
// mesh wrapper (proven by tools/crabgen/tests/golden_as.rs).
//
