# hello — crabcraft guest (Rust lane)

<!-- generated by crabgen — edits will be overwritten on regen -->

Generated by crabgen. `hello.wit` is the source of truth; `gen/`,
...
`../../modules/hello.wasm`, then the script fails hard if any SIMD (0xfd)
opcodes snuck in — the wasmcraft engine refuses them. Host-side checks run
with plain cargo: `cargo check -p hello`.
...
```yaml
name: hello
wasm: <URL serving modules/hello.wasm>
kind: reactor
...
1. Edit `hello.wit`.
2. `crabgen regen guest/hello` — rewrites the generated files and prints
