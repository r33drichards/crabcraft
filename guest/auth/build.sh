#!/usr/bin/env bash
# Build the auth reactor module (scaffolded by crabgen; edit freely — this
...
nix shell nixpkgs#rustup --command cargo build --release --target wasm32-wasip1 -p auth
cp ../../target/wasm32-wasip1/release/auth.wasm ../../modules/auth.wasm
...
disasm="$(nix shell nixpkgs#wabt --command wasm-objdump -d ../../modules/auth.wasm)"
